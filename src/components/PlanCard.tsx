...
import { Dumbbell, Flame, Leaf, Wheat, BadgePercent, Activity, Droplet, Settings2, Zap } from "lucide-react";
...
export function PlanCard({ p, bestValue = false }: { p: any; bestValue?: boolean }) {
  ...
  const goToPlan = () => nav({ to: "/plans/$id", params: { id: p.id } });
  const quickSubscribe = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    nav({ to: "/checkout", search: { plan: p.id } as any });
  };
  const customize = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    nav({ to: "/plans/$id", params: { id: p.id }, search: { customize: 1 } as any });
  };

  return (
    <Card
      onClick={goToPlan}
      className="group relative flex h-full cursor-pointer flex-col overflow-hidden rounded-2xl border shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
    >
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-3.5">
        ...
        {/* Action row: quick Add + Customize */}
        ...
        <Link
          ...
          className="text-center text-[11px] font-medium text-muted-foreground underline-offset-2 hover:text-primary hover:underline"
        >
          View plan details →
        </Link>
      </div>
